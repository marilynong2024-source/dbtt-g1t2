...
price_profile = order_items.groupby('user_id').agg(
    ...
    total_spend=('line_total_sgd',     'sum')
).reset_index()

# ── Step 2: Classify each user into a price tier ─────────────
# Price Sensitive  → buys mostly on promotion (>60% promo rate)
# Premium Seeker   → consistently buys higher-priced items (avg > $12)
# Value Conscious  → middle ground — occasional promo, mid-range prices
def price_tier(row):
    if row['promo_purchase_rate'] > 0.60:
        return 'Price Sensitive'
    elif row['avg_unit_price'] > 12:
...
def recommend_with_price_filter(user_id, n=5):
    """
    Combines collaborative filtering with price-sensitivity profiling.
    Step 1: Get 20 candidate recommendations via collaborative filtering.
    Step 2: Look up user's price tier.
    Step 3: Filter candidates to match price preference.
    Step 4: Fall back to unfiltered recommendations if too few remain.
    """
    # Base collaborative filtering — get a wide candidate pool
    candidates = recommend_products(user_id, n=20)
    if not candidates:
        return candidates

    # Get price tier — default to Value Conscious if user not found
    tier_row = price_profile[price_profile['user_id'] == user_id]
    tier = tier_row['price_tier'].values[0] if len(tier_row) > 0 else 'Value Conscious'

    # Filter candidates by price tier preference
    tier_pool   = tier_product_map.get(tier, value_products)
    filtered    = [p for p in candidates if p in tier_pool]

    # Fallback: if fewer than n filtered results, pad with unfiltered candidates
    if len(filtered) < n:
        extras   = [p for p in candidates if p not in filtered]
        filtered = (filtered + extras)[:n]

    return filtered[:n], tier
...
for uid in users['user_id'].iloc[:5]:
    ...
    aware, tier = recommend_with_price_filter(uid, n=3)
